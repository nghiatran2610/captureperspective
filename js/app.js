...
class App {
  ...
  _handleBaseUrlInput(event) {
    ...
    if (
      ...
      console.error("One or more required sections not found!");
      return;
    }
  ...
  _setupEventListeners() {
    ...
    if (baseUrlInput) {
      ...
      console.error("#baseUrlInput not found!");
    }
  ...
  _setupSimpleModeSettings() {
    const parentElement = document.getElementById("url-input-container");
    if (!parentElement) {
      console.error("#url-input-container not found.");
      return;
    }

    // Find the row containing the Screen Size dropdown and Full Page checkbox
    const screenSizeRow = parentElement.querySelector(".screen-size-row");
    if (!screenSizeRow) {
      console.error(".screen-size-row not found within #url-input-container.");
      return;
    }

    // Check if the wait time setting container already exists *after* the screen size row
    let waitTimeContainer = screenSizeRow.nextElementSibling;
    // Verify if the next sibling is indeed our wait time container
    if (
      !waitTimeContainer ||
      waitTimeContainer.id !== "simpleWaitTimeContainer"
    ) {
      waitTimeContainer = document.getElementById("simpleWaitTimeContainer"); // Try finding by ID anywhere else first
      // If found elsewhere, remove it before recreating
      if (waitTimeContainer) waitTimeContainer.remove();

      // --- Create the container for the wait time label and input ---
      waitTimeContainer = document.createElement("div");
      waitTimeContainer.id = "simpleWaitTimeContainer"; // Keep ID for potential future reference
      // Add a specific class for styling this setting group
      ...
      simpleWaitTime.className = "wait-time-input"; // Keep existing styling class
      simpleWaitTime.min = "1";
      simpleWaitTime.max = config.timing.maxWaitTime / 1000 || 120;
      simpleWaitTime.value =
        UI.elements.waitTime?.value || config.ui.defaultWaitTime || 5;
      waitTimeContainer.appendChild(simpleWaitTime);

      // --- Insert the wait time container *after* the screen size row ---
      screenSizeRow.insertAdjacentElement("afterend", waitTimeContainer);
    } else {
      // --- Ensure visibility and update value if it already exists ---
      waitTimeContainer.style.display = ""; // Ensure visible
      // Add the styling class if it's missing
  ...
  _checkCaptureButtonState() {
    ...
    if (UI.elements.captureForm.style.display !== "none") {
      ...
      captureBtn.disabled =
        this._processingQueue || urlSelector.selectedUrls.size === 0;
    } else {
  ...
  async _initiateUrlFetching() {
    if (!this.baseUrlValid || !urlFetcher.projectName) {
      ...
      if (
        ...
        typeof urlSelector.showFallbackUI === "function"
      ) {
  ...
  async captureScreenshots() {
    ...
    try {
      ...
      if (combineAllPdfBtn) {
        combineAllPdfBtn.style.display = pdfBtnVisible
          ? "inline-block"
          : "none";
  ...
  async processCaptureQueue() {
    ...
    if (!this._processingQueue) {
      this._processingQueue = true;
      console.log("Starting/Resuming queue.");
    }
    ...
    while (this.currentCaptureIndex < totalUrls) {
      if (this.isPaused) {
        console.log(
          `[Loop Start] Pause detected. Index: ${this.currentCaptureIndex}`
        );
        this._processingQueue = false;
        break;
      }
      ...
      try {
        if (this.isPaused) {
          console.log(
            `[Pre-Capture Check] Pause detected for URL: ${url}. Breaking.`
          );
          this._processingQueue = false;
          break;
        }
        ...
        if (this.isPaused) {
          console.log(
            `[Post-Capture Check] Pause detected for URL: ${url}. Breaking.`
          );
          this._processingQueue = false;
          break;
        }
        ...
        if (this.isPaused) {
          console.log(
            `[Capture Error Check] Pause detected for URL: ${url}. Breaking.`
          );
          this._processingQueue = false;
          break;
        }
        ...
        if (
          ...
          AppState.addFailedUrl(url);
          UI.utils.showStatus(
            `✗ Failed: ${url} (${error.message || "Unknown error"})`,
            true
          );
        }
      }
      ...
      if (this.isPaused) {
        console.log(
          `[Post-Processing Check] Pause detected. Index: ${this.currentCaptureIndex}`
        );
        this._processingQueue = false;
        break;
      }
      ...
      if (this.currentCaptureIndex < totalUrls) {
        if (this.isPaused) {
          console.log(
            `[Pre-Delay Check] Pause detected. Index: ${this.currentCaptureIndex}`
          );
          this._processingQueue = false;
          break;
        }
