...
function getElementText(element) {
  ...
  const specificTextElement = element.querySelector(
    ".ia_menuTreeComponent__item__label, .ia_labelComponent span, .label, .text"
  );
...
export async function waitForIframeLoad(iframe) {
  return new Promise((resolve) => {
    ...
    if (
      ...
      setTimeout(() => {
        console.log(
          "Resolving waitForIframeLoad for already complete iframe after delay."
        );
        resolve();
      }, 6000); // Increased delay slightly
    }
...
export async function waitForSubmenu(iframe, maxAttempts = 10, interval = 500) {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      const submenuGroup = iframe.contentDocument?.querySelector(
      ...
      if (submenuGroup) {
        ...
        if (submenuItems.length > 0) {
          ...
          const visibleItems = Array.from(submenuItems).filter(
            (item) => !item.classList.contains("item-invisible")
          );
          console.log(
            `${visibleItems.length} visible submenu items (filtered out ${
              submenuItems.length - visibleItems.length
            } invisible items)`
          );
          return visibleItems;
        }
        ...
        console.log(
          `Attempt ${
            i + 1
          }: Submenu container (.submenu-group .wrapper-submenu) not found yet.`
        );
      }
    } catch (e) {
...
export function getElementXPath(element, document) {
  ...
  for (let i = 0; i < siblings.length; i++) {
    ...
    if (sibling === element) {
      ...
      return (
        getElementXPath(element.parentNode, document) +
        "/" +
 ...
 * Prioritizes elements within the main menu structure and excludes invisible items.
...
export function findMenuElements(document) {
  ...
  if (!elements || elements.length === 0) {
    console.log(
      "Second menu selector failed, trying fallback: [class*='menu-item']"
    );
  ...
  elements.forEach((el) => {
    ...
    if (el.classList.contains("item-invisible")) {
      console.log(
        "Skipping invisible menu item:",
        el.getAttribute("data-label") || el.textContent.trim()
      );
      return;
    }
    ...
    if (
      ...
      !el.classList.contains("menu-back-action")
    ) {
      // Avoid headers/back buttons
      menuItems.push({ element: el, text: text });
    } else {
      console.log(
        "Skipping element with no text or is header/back:",
